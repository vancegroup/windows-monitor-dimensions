 ...
 * Date:     2010/07/21
 *
 * Updated in 2012 by Ryan Pavlik
...
namespace ScopedHandlePolicies {
	...
	class DeleteObjectMethod {
		public:
			...
			static inline void Close(HGDIOBJ Handle) {
				DeleteObject(Handle);
			}
	};
	...
	class CloseSocketMethod {
		public:
			...
			static inline void Close(SOCKET Handle) {
				closesocket(Handle);
			}
	};
	...
	class UnmapViewOfFileMethod {
		public:
			...
			static inline void Close(LPVOID Handle) {
	...
	class RegCloseKeyMethod {
		public:
			...
			static inline void Close(HKEY Handle) {
				RegCloseKey(Handle);
			}
	};
...
namespace boost {
	/// @brief Boost-linked get_pointer support for these scoped handles.
	template <typename HandleType, class InvalidValue, class CloseMethod>
	HandleType get_pointer(ScopedHandle<HandleType, InvalidValue, CloseMethod> const & ptr) {
		return ptr.Get();
	}
}

/// The wrapper for the event handle returned from CreateEvent() method.
typedef ScopedHandle<HANDLE, ScopedHandlePolicies::NullHandleValue<HANDLE>, ScopedHandlePolicies::CloseHandleMethod> EventScopedHandle;

/// The wrapper for the thread handle returned from CreateThread() method.
typedef ScopedHandle<HANDLE, ScopedHandlePolicies::NullHandleValue<HANDLE>, ScopedHandlePolicies::CloseHandleMethod> ThreadScopedHandle;

/// The wrapper for the file handle returned from CreateFile() method.
typedef ScopedHandle<HANDLE, ScopedHandlePolicies::InvalidHandleValue, ScopedHandlePolicies::CloseHandleMethod> FileScopedHandle;

/// The wrapper for the socket handle returned from socket() method.
typedef ScopedHandle<SOCKET, ScopedHandlePolicies::InvalidSocketValue, ScopedHandlePolicies::CloseSocketMethod> SocketScopedHandle;

/// The wrapper for the file mapping handle returned from CreateFileMapping() method.
typedef ScopedHandle<HANDLE, ScopedHandlePolicies::NullHandleValue<HANDLE>, ScopedHandlePolicies::CloseHandleMethod> MappingScopedHandle;

/// The wrapper for the view of file handle returned from MapViewOfFile() method.
typedef ScopedHandle<LPVOID, ScopedHandlePolicies::NullHandleValue<LPVOID>, ScopedHandlePolicies::UnmapViewOfFileMethod> ViewOfFileScopedHandle;

/// The wrapper for the registry key handle returned from RegOpenKeyEx() method.
typedef ScopedHandle<HKEY, ScopedHandlePolicies::NullHandleValue<HKEY>, ScopedHandlePolicies::RegCloseKeyMethod> RegKeyScopedHandle;

/// The wrapper for the module handle returned from LoadLibrary() method.
typedef ScopedHandle<HMODULE, ScopedHandlePolicies::NullHandleValue<HMODULE>, ScopedHandlePolicies::FreeLibraryMethod> ModuleScopedHandle;

/// The wrapper for the find file handle returned from FindFirstFile() method.
typedef ScopedHandle<HANDLE, ScopedHandlePolicies::InvalidHandleValue, ScopedHandlePolicies::FindCloseMethod> FindFileScopedHandle;
